...
const Sidebar = () => {
  ...
  return (
    <>
        <aside className='hidden md:flex transition-all w-[230px]  px-4 duration-300  flex-col bg-[#f3f4f6] justify-between z-50 h-screen border-r border-[#dadada]'>
            <div className=''>
                <Link to='/'>
                    ...
                    {sidebarData.routesPath.map((route,index) =>{
                        ...
                        return (
                            <NavLink to={route} key={route}>{({ isActive }) => (
                                <div className={`flex items-center text-[#364153] gap-x-4 p-2 w-full rounded-md ${isActive ? 'bg-[#bdf094]' : ''}`}>
                                    <Icon className={`${isActive ? 'text-black' :'text-[#364153] '}text-xl`} />
                                    {/* <Icon className="text-2xl text-gray-600 pr-1" /> */}
                                    <p className={`${isActive ? 'text-black' : 'text-[#364153]'}`}>{sidebarData.routesName[index]}</p>
                                </div>
                            )}
                            </NavLink>
